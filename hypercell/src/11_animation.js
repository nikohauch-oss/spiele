...
(function (HC, THREE) {
  ...
  HC.Animator = function Animator(model, charDef) {
    ...
    const POSES = {
      lobby(P, t, w) {
        addPose(P, 'spine', 0.03, -0.10, 0, w); addPose(P, 'chest', 0.02, -0.14, 0, w);
        addPose(P, 'armUpperR', -0.55, -0.15, -0.30, w); addPose(P, 'armLowerR', -0.65, 0, 0, w);
        addPose(P, 'armUpperL', -0.42, 0.15, 0.34, w); addPose(P, 'armLowerL', -0.60, 0, 0, w);
        addPose(P, 'thighL', 0, 0, 0.09, w); addPose(P, 'thighR', -0.10, 0, -0.12, w);
        addPose(P, 'head', 0, 0.08, 0, w);
      },
      select(P, t, w) {
        const k = U.pulse(U.clamp01(t * 1.6), 0.35);
        addPose(P, 'chest', -0.10 * k, -0.20, 0, w); addPose(P, 'spine', -0.06 * k, -0.08, 0, w);
        addPose(P, 'armUpperR', -0.95 - k * 0.55, -0.24, -0.34, w);
        addPose(P, 'armLowerR', -0.85, 0, 0.1, w);
        addPose(P, 'armUpperL', -1.15, 0.42, 0.48, w); addPose(P, 'armLowerL', -1.25, 0, -0.28, w);
        addPose(P, 'head', -0.06, -0.10, 0, w);
        addPose(P, 'thighR', -0.14, 0, -0.14, w); addPose(P, 'shinR', 0.10, 0, 0, w);
      },
      victory(P, t, w) {
