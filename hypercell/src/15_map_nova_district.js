...
(function (HC, THREE) {
  ...
  function buildNovaDistrict(scene, world, options) {
    ...
    /* --- rooftop access --- */
    // Long ramps from the street up to the 7m shop roofs
    [-1, 1].forEach(sx => {
      ramp(sx * 39.5, 0.18, -22, 5.0, 14, 6.82, 'x', sx < 0, M.panel, 'metal');
      ramp(sx * 39.5, 0.18, 22, 5.0, 14, 6.82, 'x', sx < 0, M.panel, 'metal');
    });
    ...
    [-1, 1].forEach(sx => {
      ...
      ramp(sx * 25.8, 0.18, 0, 4.4, 12, 7.32, 'x', sx > 0, M.panel, 'metal');
    });
    ...
    function tunnel(sx) {
      ...
      const depth = -3.6;
      // Trench walls
      solid(x - 3.6, depth / 2 - 0.5, 0, 1.2, 4.6, 26, M.wallDark, 'concrete');
      solid(x + 3.6, depth / 2 - 0.5, 0, 1.2, 4.6, 26, M.wallDark, 'concrete');
      solid(x, depth - 0.4, 0, 6.0, 0.8, 26, M.sidewalk, 'concrete');
      // Ceiling over the middle section makes it a true tunnel, not a ditch.
      solid(x, 0.4, 0, 8.0, 0.8, 14, M.wallDark, 'concrete');
      for (let z = -5; z <= 5; z += 2.5) {
        deco(new THREE.BoxGeometry(5.4, 0.10, 0.35), M.windowCool, [x, -0.15, z]);
        lights.push(makePointLight(x, -0.5, z, 0x62d6ff, 0.55, 11));
      }
      // Pipe runs
      [-1, 1].forEach(s => {
        deco(new THREE.CylinderGeometry(0.20, 0.20, 25, 8), M.rust, [x + s * 2.9, -0.9, 0], [Math.PI / 2, 0, 0]);
        deco(new THREE.CylinderGeometry(0.14, 0.14, 25, 8), M.darkSteel, [x + s * 2.9, -1.5, 0], [Math.PI / 2, 0, 0]);
      });
      // Entrance ramps at both ends
      ramp(x, depth, -16.5, 5.6, 7, 3.78, 'z', false, M.sidewalk, 'concrete');
      ramp(x, depth, 16.5, 5.6, 7, 3.78, 'z', true, M.sidewalk, 'concrete');
      result.ambientEmitters.push({ pos: new THREE.Vector3(x, -1.2, 0), sound: 'vent_hiss', interval: 6.5 });
    }
    tunnel(-1);
    tunnel(1);
